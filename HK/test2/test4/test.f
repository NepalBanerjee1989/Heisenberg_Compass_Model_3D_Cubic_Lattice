...
C==================================================C
         ...
         call Random_Initial_Heisenberg_spin_simulation
         call Random_Initial_Spin_Configuration_DATA
...
C-------------------------C
         ...
         call Metropolish_spin_flipping
   ...
   20   continue
         call OUTPUT_DATA
C         if(it.eq.it2)then
         call Final_Spin_Configuration_After_transition_DATA
         if(it.eq.it2)then
         write(*,5) 
         end if
   30    continue
...
C=============================================C
      ...
      SUBROUTINE Random_Initial_Heisenberg_spin_simulation
             IMPLICIT NONE
             INTEGER i,j,k,L
             PARAMETER(L=6)
             REAL*8 pi,S,th,phi,
...
C             th = pi/2   
    ...
    2         CONTINUE
      ...
      SUBROUTINE Metropolish_spin_flipping
         IMPLICIT NONE
     ...
     & S,E1,E1H,E1K,dE,prob,t
        ...
        COMMON /SPINS/ hs1, hs2, hs3,flip1,flip2,flip3
C***Simulating Random Heisenberg spin****C
         pi=4*atan(1.d0) ;
         ...
         j=1+rand()*L
         k=1+rand()*L
         call Periodic_Boundary_Condition
...
C***Simulating Random Heisenberg spin****C
  ...
  5    continue
      ...
      SUBROUTINE Periodic_Boundary_Condition
      IMPLICIT NONE
...
***periodicity of lattice define here****
      ...
      SUBROUTINE Random_Initial_Spin_Configuration_DATA
      INTEGER i,j,k,L
      PARAMETER(L=6)
      REAL*8  hs1(L,L,L),hs2(L,L,L),hs3(L,L,L)
  ...
  6   continue
      ...
      SUBROUTINE Final_Spin_Configuration_After_transition_DATA
      INTEGER i,j,k,L
      PARAMETER(L=6)
